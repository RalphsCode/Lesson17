...
async function getFavorites() {
  if (currentUser !== undefined) {
    ...
    console.log(
      "user favorites array length: ",
      response.data.user.favorites.length
    );
  ...
  if (currentUser !== undefined) {
    ...
    if (userFavoritesArr !== undefined) {
      if (userFavoritesArr.length >= 1) {
        ...
        const foundFavorite = userFavoritesArr.find(
...
function showDelete() {
  if (currentUser === undefined) {
    ...
    return '&nbsp; <i class="fa-solid fa-xmark" style="color: lightcoral;" title="Delete story" id="story-delete"></i>&nbsp;';
  }
} // End showDelete
...
async function putStoriesOnPage(evt) {
  ...
  if (evt != undefined) {
    // exclude fresh page load which doesn't have an event
    const whichStories = evt.target.id; // get the type of stories requested
    if (whichStories == "nav-favorites") {
      // user requested favorite stories
      if (currentUser !== undefined) {
        if (userFavoritesArr == undefined || userFavoritesArr.length == 0) {
          // if ther are no favorite stories
          const noFavoriteText =
            "<p>You have not tagged any stories as favorite stories.</p>";
